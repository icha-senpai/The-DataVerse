<?php
// FROM HASH: 689abd864f2bebe49dcf38c11e3bfaea
return array(
...
';
	if (!$__templater->test($__vars['stats'], 'empty', array())) {
		$__finalCompiled .= '
	';
...
';
	...
	<div class="block">
		...
		if ($__vars['envReport']['phpVersionState'] === 'not_newest') {
			$__finalCompiled .= '
					<div class="block-rowMessage block-rowMessage--warning">
						' . 'Your current version of PHP is outdated. We currently recommend PHP  ' . $__templater->escape($__vars['envReport']['phpVersionRecommended']) . '.' . '
					</div>
				';
		...
		$__finalCompiled .= '
					...
					<dl class="pairs pairs--columns">
						<dt>' . 'Snuffleupagus enabled' . '</dt>
						<dd>' . ($__vars['envReport']['snuffleupagus'] ? 'Yes' : 'No') . '</dd>
					</dl>
					<dl class="pairs pairs--columns">
					...
					<dl class="pairs pairs--columns">
						<dt>' . '<code>intl</code> support' . '</dt>
						<dd>' . ($__vars['envReport']['intl'] ? 'Yes' : 'No') . '</dd>
					</dl>
					<dl class="pairs pairs--columns">
...
';
	if (!$__templater->test($__vars['logCounts'], 'empty', array())) {
		$__finalCompiled .= '
	<div class="block">
		<div class="block-container">
			<h2 class="block-header">
	...
	<div class="block">
		<div class="block-container">
			...
			<div class="block-body block-body--collapsible ' . ((!$__templater->func('is_toggled', array('collapse_health_check', ), false)) ? 'is-active' : '') . '" id="js-collapse-health-check">
				' . $__templater->callMacro(null, 'tools_file_check::file_check_list', array(
...
';
	if (!$__templater->test($__vars['installedAddOns'], 'empty', array())) {
		$__finalCompiled .= '
	';
	...
	<div class="addOnList">
		...
		if ($__templater->isTraversable($__vars['installedAddOns'])) {
			foreach ($__vars['installedAddOns'] AS $__vars['addOn']) {
				$__finalCompiled .= '
						' . $__templater->callMacro(null, 'addon_list_macros::addon_list_item', array(
					'addOn' => $__vars['addOn'],
				), $__vars) . '
					';
			}
