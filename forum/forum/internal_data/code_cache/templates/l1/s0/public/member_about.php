<?php
// FROM HASH: c3a5c07c525b372259749293157b80b5
return array(
...
<div class="block">
	...
	if ($__templater->method($__vars['user'], 'canViewIdentities', array())) {
		...
		if ($__templater->method($__vars['xf']['visitor'], 'canStartConversationWith', array($__vars['user'], ))) {
			$__compilerTemp4 .= '
								<dl class="pairs pairs--columns pairs--fixedSmall">
									<dt>' . 'Direct message' . '</dt>
									<dd><a href="' . $__templater->func('link', array('direct-messages/add', '', array('to' => $__vars['user']['username'], ), ), true) . '">' . 'Send direct message' . '</a></dd>
								</dl>
							';
		}
		$__compilerTemp4 .= '
	...
	if ($__vars['followingCount']) {
		...
		if (strlen(trim($__compilerTemp5)) > 0) {
			...
			if ($__vars['followingCount'] > $__templater->func('count', array($__vars['following'], ), false)) {
				$__compilerTemp1 .= '
							<a href="' . $__templater->func('link', array('members/following', $__vars['user'], ), true) . '" rel="nofollow" data-xf-click="overlay">' . '... and ' . ($__vars['followingCount'] - $__templater->func('count', array($__vars['following'], ), false)) . ' more.' . '</a>
						';
			}
			$__compilerTemp1 .= '
					</div>
	...
	if ($__vars['followersCount']) {
		...
		if (strlen(trim($__compilerTemp6)) > 0) {
			...
			if ($__vars['followersCount'] > $__templater->func('count', array($__vars['followers'], ), false)) {
				$__compilerTemp1 .= '
							<a href="' . $__templater->func('link', array('members/followers', $__vars['user'], ), true) . '" rel="nofollow" data-xf-click="overlay">' . '... and ' . ($__vars['followersCount'] - $__templater->func('count', array($__vars['followers'], ), false)) . ' more.' . '</a>
						';
			}
			$__compilerTemp1 .= '
					</div>
