<?php
// FROM HASH: de70ddbf5b3198071e1c711d611d6f85
return array(
	...
	if ($__templater->isTraversable($__vars['recent'])) {
		foreach ($__vars['recent'] AS $__vars['shortname'] => $__vars['emoji']) {
			...
			if ($__vars['emoji']['smilie_id']) {
				$__finalCompiled .= '
						' . $__templater->func('smilie', array($__vars['shortname'], 'rte', ), true) . '
					';
			} else {
				$__finalCompiled .= '
		...
		';
			...
			if ($__templater->isTraversable($__vars['groupedSmilies'][$__vars['categoryId']])) {
				foreach ($__vars['groupedSmilies'][$__vars['categoryId']] AS $__vars['smilie']) {
					$__compilerTemp1 .= '
							<li><a class="js-emoji" data-shortname="' . $__templater->escape($__vars['smilie']['smilie_text_options']['0']) . '">' . $__templater->func('smilie', array($__vars['smilie']['smilie_text_options']['0'], 'rte', ), true) . '</a></li>
						';
				}
			}
			$__compilerTemp1 .= '
