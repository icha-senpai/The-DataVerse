<?php
// FROM HASH: 47f80e9e82099b3a17f0b32241b09715
return array(
...
';
	...
	<div class="bbCodeBlock bbCodeBlock--unfurl ' . ($__vars['result']['pending'] ? 'is-pending' : '') . ' ' . ($__vars['result']['is_recrawl'] ? 'is-recrawl' : '') . ' ' . ($__vars['simple'] ? 'is-simple' : '') . ' js-unfurl fauxBlockLink"
		...
		if ((($__vars['result']['pending'] OR $__vars['imageUrl'])) AND (!$__vars['simple'])) {
			...
			if ($__vars['imageUrl']) {
				$__finalCompiled .= '
						<img src="' . $__templater->escape($__vars['imageUrl']) . '" loading="lazy" alt="' . $__templater->escape($__vars['result']['host']) . '" class="bbCodeBlockUnfurl-image" data-onerror="hide-parent"/>
					';
			} else {
				$__finalCompiled .= '
		...
		if ($__vars['faviconUrl']) {
			$__finalCompiled .= '
							<img src="' . $__templater->escape($__vars['faviconUrl']) . '" loading="lazy" alt="' . $__templater->escape($__vars['result']['host']) . '" class="bbCodeBlockUnfurl-icon"
