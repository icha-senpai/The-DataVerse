<?php
// FROM HASH: 547fe4d44b699c222cf20574413251e7
return array(
...
'thread_action_buttons' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['canInlineMod']) {
		$__finalCompiled .= '
							' . $__templater->callMacro(null, 'inline_mod_macros::button', array(), $__vars) . '
						';
	}
	$__finalCompiled .= '
						';
	...
	$__compilerTemp2 .= '
											';
	if ($__templater->method($__vars['thread'], 'canFeatureUnfeature', array())) {
		$__compilerTemp2 .= '
												<a href="' . $__templater->func('link', array('threads/feature', $__vars['thread'], ), true) . '" class="menu-linkRow"
													data-xf-click="overlay">

													';
		if ($__templater->method($__vars['thread'], 'isFeatured', array())) {
			$__compilerTemp2 .= '
														' . 'Manage feature' . '
													';
		} else {
...
' . $__templater->callMacro(null, 'metadata_macros::oembed', array(
		'oembedUrl' => $__templater->method($__vars['thread'], 'getOembedEndpointUrl', array()),
		'title' => $__templater->method($__vars['thread'], 'getContentTitle', array()),
	), $__vars) . '
...
';
	...
	';
		$__compilerTemp4 = '';
		$__compilerTemp4 .= $__templater->func('prefix_description', array('thread', $__vars['thread']['prefix_id'], ), true);
...
' . $__templater->callMacro(null, 'forum_macros::forum_page_options', array(
		'forum' => $__vars['forum'],
		'thread' => $__vars['thread'],
	), $__vars) . '
...
';
	...
	';
		$__templater->includeJs(array(
			'src' => 'xf/action.js, xf/message.js, xf/captcha.js',
			'min' => '1',
		));
		...
		<div class="block-container">
			...
			'lastItemId' => $__vars['lastPost']['post_id'],
