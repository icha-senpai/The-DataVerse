<?php
// FROM HASH: 61f25a70fd5556c5bf4e1fe7f0b1fafe
return array(
...
<div class="block">
	...
	$__compilerTemp5 = '';
	$__compilerTemp5 .= '
																' . $__templater->callMacro(null, 'member_macros::moderator_menu_actions', array(
		'user' => $__vars['user'],
		'context' => 'profile',
	), $__vars) . '
															';
				...
				<div class="memberHeader-content">
					<div class="memberHeader-stats">
						<div class="pairJustifier">
							' . $__templater->callMacro(null, 'member_macros::member_stat_pairs', array(
		'user' => $__vars['user'],
		'context' => 'profile',
	), $__vars) . '
						</div>
...
<ul class="tabPanes js-memberTabPanes">
	...
	if ($__templater->method($__vars['user'], 'canViewPostsOnProfile', array())) {
		...
		if ($__templater->method($__vars['user'], 'canPostOnProfile', array())) {
			...
			$__finalCompiled .= '
							' . $__templater->callMacro(null, 'profile_post_macros::quick_post', array(
				'user' => $__vars['user'],
		...
		if (!$__templater->test($__vars['profilePosts'], 'empty', array())) {
			...
			if ($__templater->isTraversable($__vars['profilePosts'])) {
				foreach ($__vars['profilePosts'] AS $__vars['profilePost']) {
					$__finalCompiled .= '
								' . $__templater->callMacro(null, 'profile_post_macros::' . (($__vars['profilePost']['message_state'] == 'deleted') ? 'profile_post_deleted' : 'profile_post'), array(
			...
			if (!$__templater->test($__vars['newsFeedItems'], 'empty', array())) {
				...
				if ($__templater->isTraversable($__vars['newsFeedItems'])) {
					foreach ($__vars['newsFeedItems'] AS $__vars['item']) {
						$__finalCompiled .= '
									' . $__templater->callMacro(null, 'news_feed_macros::feed_row', array(
							'item' => $__vars['item'],
						), $__vars) . '
								';
					}
