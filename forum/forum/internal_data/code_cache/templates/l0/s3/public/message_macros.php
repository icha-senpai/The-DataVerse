<?php
// FROM HASH: 9a172071ef33dab648763b4b950c925d
return array(
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	<section class="message-user ' . ((!$__vars['__globals']['uix_postbitCollapsed']) ? 'userExtra--expand' : '') . '"
			 ' . ((($__vars['includeMicrodata'] AND $__vars['itemProp'])) ? (('itemprop="' . $__templater->escape($__vars['itemProp'])) . '"') : '') . '
			 ' . ($__vars['includeMicrodata'] ? 'itemscope itemtype="https://schema.org/Person"' : '') . '
			 ' . ((($__vars['includeMicrodata'] AND $__vars['user'])) ? (('itemid="' . $__templater->func('link', array('canonical:members', $__vars['user'], ), true)) . '"') : '') . '>
	...
	if ($__vars['user']['user_id']) {
		...
		if ($__vars['extras']['website'] AND $__vars['user']['Profile']['website']) {
			...
			$__compilerTemp1 .= '
											<dd data-xf-init="tooltip" title="' . $__templater->filter($__vars['user']['Profile']['website'], array(array('url', array('host', 'Visit site', )),), true) . '"><a href="' . $__templater->escape($__vars['user']['Profile']['website']) . '" rel="nofollow" target="_blank">' . $__templater->filter($__vars['user']['Profile']['website'], array(array('url_display', array('host', 'Visit site', )),), true) . '</a></dd>
										</dl>
									';
		}
		$__compilerTemp1 .= '
		...
		if ($__vars['extras']['custom_fields']) {
			$__compilerTemp1 .= '
										' . $__templater->callMacro(null, 'custom_fields_macros::custom_fields_values', array(
				'type' => 'users',
				'group' => 'personal',
	...
	if ($__templater->isTraversable($__vars['attachments'])) {
		foreach ($__vars['attachments'] AS $__vars['attachment']) {
			if (!$__templater->method($__vars['message'], 'isAttachmentEmbedded', array($__vars['attachment'], ))) {
				$__compilerTemp1 .= '
						' . $__templater->callMacro(null, 'attachment_macros::attachment_list_item', array(
