<?php
// FROM HASH: 84ea96a4d0d75bd06675e72ad6ebf151
return array(
'code' => function($__templater, array $__vars, $__extensions = null)
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Direct messages');
...
' . $__templater->callMacro(null, 'conversation_list_macros::conversation_page_options', array(), $__vars) . '

';
	if ($__templater->method($__vars['xf']['visitor'], 'canStartConversation', array())) {
		$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	' . $__templater->button('Send direct message', array(
			'href' => $__templater->func('link', array('direct-messages/add', ), false),
			'class' => 'button--cta',
			'icon' => 'write',
		), '', array(
		)) . '
...
<div class="block" data-xf-init="inline-mod" data-type="conversation" data-href="' . $__templater->func('link', array('inline-mod', ), true) . '">
	<div class="block-outer">
		...
		<div class="block-outer-opposite">
			<div class="buttonGroup">
				' . $__templater->callMacro(null, 'inline_mod_macros::button', array(
	...
	if ($__vars['filters']['starter_id'] AND $__vars['starterFilter']) {
		$__compilerTemp1 .= '
							<li><a href="' . $__templater->func('link', array('direct-messages', null, $__templater->filter($__vars['filters'], array(array('replace', array('starter_id', null, )),), false), ), true) . '"
	...
	if ($__vars['filters']['receiver_id'] AND $__vars['receiverFilter']) {
		$__compilerTemp1 .= '
							<li><a href="' . $__templater->func('link', array('direct-messages', null, $__templater->filter($__vars['filters'], array(array('replace', array('receiver_id', null, )),), false), ), true) . '"
								class="filterBar-filterToggle" data-xf-init="tooltip" title="' . $__templater->filter('Remove this filter', array(array('for_attr', array()),), true) . '">
	...
	if ($__vars['filters']['starred']) {
		$__compilerTemp1 .= '
							<li><a href="' . $__templater->func('link', array('direct-messages', null, $__templater->filter($__vars['filters'], array(array('replace', array('starred', null, )),), false), ), true) . '"
								class="filterBar-filterToggle" data-xf-init="tooltip" title="' . $__templater->filter('Remove this filter', array(array('for_attr', array()),), true) . '">
	...
	if ($__vars['filters']['unread']) {
		$__compilerTemp1 .= '
							<li><a href="' . $__templater->func('link', array('direct-messages', null, $__templater->filter($__vars['filters'], array(array('replace', array('unread', null, )),), false), ), true) . '"
								class="filterBar-filterToggle" data-xf-init="tooltip" title="' . $__templater->filter('Remove this filter', array(array('for_attr', array()),), true) . '">
								<span class="filterBar-filterToggle-label">' . 'Show only' . $__vars['xf']['language']['label_separator'] . '</span>
								' . 'Unread' . '</a></li>
						';
	...
	$__finalCompiled .= '
		...
		'label' => 'Show all direct messages',
		'_type' => 'option',
	),
	array(
		'value' => 'started',
	...
	array(
		...
		'class' => 'button--primary',
						...
						', array(
		'action' => $__templater->func('link', array('direct-messages/filters', ), false),
	)) . '
					</div>
				</div>
			</div>
	...
	if (!$__templater->test($__vars['userConvs'], 'empty', array())) {
		...
		if ($__templater->isTraversable($__vars['userConvs'])) {
			foreach ($__vars['userConvs'] AS $__vars['userConv']) {
				$__finalCompiled .= '
						' . $__templater->callMacro(null, 'conversation_list_macros::item', array(
		...
		$__finalCompiled .= '
				<div class="block-row">' . 'There are no direct messages to display.' . '</div>
			';
	}
	$__finalCompiled .= '
		</div>
	...
	if ($__templater->method($__vars['xf']['visitor'], 'canStartConversation', array())) {
		$__finalCompiled .= '
			<div class="block-footer-opposite">
				<a href="' . $__templater->func('link', array('conversations/add', ), true) . '">' . 'Send direct message' . '</a>					
			</div>
		';
	}
	$__finalCompiled .= '
