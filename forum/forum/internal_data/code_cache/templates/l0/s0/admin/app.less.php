<?php
// FROM HASH: e44bd471f76d972871c4b3c668fa6bcf
return array(
...
.m-thinScrollbar(@width; @background: rgba(0, 0, 0, 0.25); @foreground: rgba(255, 255, 255, 0.4))
{
	.has-no-hiddenscroll &
	{
		...
		{
			border: calc(@width / 4) solid transparent;
		...
		{
			background-color: darken(@foreground, 30%);
		}
	}
}
...
.u-anchorTarget
{
	...
	margin-top: ((@_adminHeader-height + @_adminHeader-shadowHeight + @_adminHeader-offset) * -1);
}
...
{
	min-height: 100vh;
	width: (@_adminNav-width + @_adminNav-scroll-width);
...
.p-nav-inner
{
	...
	height: calc(100vh - @_adminHeader-height);
	overflow: clip auto;
	.m-thinScrollbar(@_adminNav-scroll-width);
}
...
.p-nav-sectionHeader
{
	...
	.p-nav-sectionToggle
	{
		...
		{
			font-size: 80%;
			.m-faContent(@fa-var-solid-chevron-down); //, 1em
		}

		.p-nav-section.is-active &:after
		{
			.m-faContent(@fa-var-solid-chevron-up); //, 1em
		}
	}
}
...
@_adminFooter-bg: darken(@_adminHeader-bg, 8%);
...
// sticky labels for formRows
.formRow.formRow--stickyLabel .formRow-labelWrapper
{
	position: sticky;
	top: @_adminHeader-height + @xf-formRowPaddingV;
}';
