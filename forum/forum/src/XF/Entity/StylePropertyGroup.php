 ...
 * @property Phrase|string $description
 * @property-read Phrase|string $master_title
 * @property-read Phrase|string $master_description
 *
 * RELATIONS
...
class StylePropertyGroup extends Entity
{
	...
	protected function _preSave()
	{
		...
		{
			$existingGroup = $this->em()->findOne(StylePropertyGroup::class, [
	...
	protected function _postSave()
	{
		...
		if ($this->isUpdate() && $this->isChanged('group_name'))
		{
			...
			$properties = $this->finder(StylePropertyFinder::class)->where([
				'style_id' => $this->style_id,
				'group_name' => $this->getExistingValue('group_name'),
			])->fetch();
			foreach ($properties AS $property)
			{
				$property->getBehavior(DevOutputWritable::class)->setOption('write_dev_output', $writeDevOutput);
