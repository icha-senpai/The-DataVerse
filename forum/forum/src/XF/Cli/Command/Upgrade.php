...
class Upgrade extends AbstractCommand implements CustomAppCommandInterface
{
	use JobRunnerTrait;
	...
	protected function execute(InputInterface $input, OutputInterface $output)
	{
		...
		if (!$upgradeComplete)
		{
			...
			$class = $app->extendClass(\XF\Install\Controller\Upgrade::class);
			$controller = new $class($app, $app->request());
		...
		if ($upgrader->isUpgradeComplete())
		{
			...
			else if (!$upgradeComplete)
			{
				...
				$outdatedTemplates = $app->repository(TemplateRepository::class)->countOutdatedTemplates();
