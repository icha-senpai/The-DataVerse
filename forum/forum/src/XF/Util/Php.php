...
class Php
{
	...
	public static function nameIndicatesReadOnly($name, $argumentCount = 0)
	{
		...
		if (preg_match('/^(' . implode('|', $matches) . ')$/', $name, $matches))
		{
			...
			{
				\XF::logError(sprintf(
					"Method '%s' with arguments will no longer be considered read-only in XF 2.3.8+",
					$name
				));
			}
		...
		if (preg_match('/^fetch[A-Z]/', $name))
		{
			\XF::logError(sprintf(
				"Method '%s' will no longer be considered read-only in XF 2.3.8+",
				$name
			));
