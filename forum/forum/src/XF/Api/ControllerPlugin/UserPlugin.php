...
class UserPlugin extends AbstractPlugin
{
	...
	public function saveEditFormFromTableInput(FormAction $form, User $user, array $tableInput)
	{
		...
		$form->basicEntitySave($user, Arr::filterNull($tableInput['user']));

		if (isset($tableInput['option']))
		{
			$userOptions = $user->getRelationOrDefault('Option');
			$form->setupEntityInput($userOptions, Arr::filterNull($tableInput['option']));
		}

		if (isset($tableInput['profile']))
		{
			$userProfile = $user->getRelationOrDefault('Profile');
			$form->setupEntityInput($userProfile, Arr::filterNull($tableInput['profile']));
		}

		if (isset($tableInput['privacy']))
		{
			$userPrivacy = $user->getRelationOrDefault('Privacy');
			$form->setupEntityInput($userPrivacy, Arr::filterNull($tableInput['privacy']));
		}
	}
	 ...
	 */
	public function userSaveProcessAdmin(User $user)
	{
		...
		$dob = Arr::filterNull($dobInput['dob']);
		if ($dob)
		{
	 ...
	 */
	public function actionUpdateAvatar(User $user)
	{
