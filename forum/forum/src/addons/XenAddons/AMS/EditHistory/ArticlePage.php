...
{
	/**
	 * @param \XenAddons\AMS\Entity\ArticlePage $content
	 */
	...
	{
		return \XF::app()->router()->buildLink('ams/pages', $content);
	}
	...
	public function revertToVersion(Entity $content, \XF\Entity\EditHistory $history, \XF\Entity\EditHistory $previous = null)
	{
		...
		$editor = \XF::app()->service('XenAddons\AMS:Page\Edit', $content);

		$editor->logEdit(false);
		$editor->setMessage($history->old_text);

		if (!$previous || $previous->edit_user_id != $content->Article->user_id)
		{
			$content->last_edit_date = 0;
		}
		else if ($previous && $previous->edit_user_id == $content->Article->user_id)
		{
	...
	{
		return \XF::app()->templater()->func('bb_code', [$text, 'ams_page', $content]);
	}
