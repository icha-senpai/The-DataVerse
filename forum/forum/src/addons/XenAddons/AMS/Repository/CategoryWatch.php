...
{
	public function setWatchState(\XenAddons\AMS\Entity\Category $category, \XF\Entity\User $user, $action, array $config = [])
	{
		...
		$watch = $this->em->find('XenAddons\AMS:CategoryWatch', [
		...
		switch ($action)
		{
			...
			case 'watch':
				if (!$watch)
				{
					$watch = $this->em->create('XenAddons\AMS:CategoryWatch');
	...
	public function setWatchStateForAll(\XF\Entity\User $user, $action, array $updates = [])
	{
		...
		switch ($action)
		{
			case 'update':
				...
				return $db->update('xf_xa_ams_category_watch', $updates, 'user_id = ?', $user->user_id);

			case 'delete':
				return $db->delete('xf_xa_ams_category_watch', 'user_id = ?', $user->user_id);
