...
use XFMG\Import\Data\Category;
use XFMG\Import\Data\Comment;
use XFMG\Import\Data\HasWatchTrait;
use XFMG\Import\Data\MediaField;
use XFMG\Import\Data\MediaItem;
use XFMG\Import\Data\MediaNote;
use XFMG\Import\Data\Rating;
...
class XFMG2 extends AbstractMGImporter
{
	...
	public function stepAlbums(StepState $state, array $stepConfig, $maxTime)
	{
		...
		foreach ($albums AS $album)
		{
			...
			/** @var Album $import */
			$import = $this->newHandler(Album::class);
	...
	public function stepMediaFields(StepState $state)
	{
		...
		foreach ($fields AS $oldId => $field)
		{
			if (!empty($existingFields[$oldId]))
			{
				...
				$this->logHandler(MediaField::class, $oldId, $oldId);
			}
			else
			{
				/** @var MediaField $import */
				$import = $this->setupCustomFieldImport(MediaField::class, $field);
		...
		foreach ($mediaItems AS $mediaItem)
		{
			...
			if ($mediaNotes)
			{
				...
				foreach ($mediaNotes AS $mediaNote)
				{
					...
					/** @var MediaNote $noteImport */
					$noteImport = $this->newHandler(MediaNote::class);
	...
	public function stepRatings(StepState $state, array $stepConfig, $maxTime)
	{
		...
		foreach ($ratings AS $rating)
		{
			...
			/** @var Rating $import */
			$import = $this->newHandler(Rating::class);
	...
	public function stepComments(StepState $state, array $stepConfig, $maxTime)
	{
		...
		foreach ($comments AS $comment)
		{
			...
			$import->bulkSet([
				'message' => $this->rewriteMessage($comment['message'], 'xfmg_comment'),
				'content_id' => $contentId,
				'user_id' => $userId,
			...
			if ($comment['edit_count'])
			{
				...
				if ($edits)
				{
					foreach ($edits AS $edit)
					{
						...
						$editHistory = $this->newHandler(EditHistory::class);
	...
	/**
	 * @param HasWatchTrait $import
	 * @param $oldId
	 */
	protected function handleWatchers(AbstractData $import, $oldId)
	{
		/** @var Watch $watchHelper */
		$watchHelper = $this->getDataHelper(Watch::class);

		[$columnName, $tableName] = $watchHelper->getTableConfigForType($import->getImportType());
