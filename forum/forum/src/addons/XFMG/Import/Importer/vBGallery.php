...
class vBGallery extends AbstractMGImporter
{
	...
	protected const OPTION_VIEW = 0x01;
	protected const OPTION_UPLOAD = 0x02;
	protected const OPTION_REPLY = 0x04;
	protected const OPTION_RATE = 0x08;
	...
	protected function getAttachPathConfig(array &$baseConfig, Adapter $sourceDb)
	{
		try
		{
	...
	protected function importCategoryTree(array $categories, array $tree, $oldParentId = 0, $newParentId = 0)
	{
		...
		foreach ($tree[$oldParentId] AS $oldCategoryId)
		{
			...
			/** @var Category $categoryImport */
			$categoryImport = $this->newHandler(Category::class);
	...
	public function stepMediaFields(StepState $state)
	{
		...
		foreach ($fields AS $oldId => $field)
		{
			...
			$import = $this->newHandler(MediaField::class);
		...
		foreach ($mediaItems AS $mediaItem)
		{
			...
			foreach ($mapFieldIds AS $oldFieldId => $newFieldId)
			{
				...
				if (isset($mediaItem[$fieldName]) && $mediaItem[$fieldName] !== '')
				{
					...
					if (!empty($fieldValue))
					{
						$fieldValues[$newFieldId] = Arr::htmlSpecialCharsDecodeArray($fieldValue);
					}
				}
			}
