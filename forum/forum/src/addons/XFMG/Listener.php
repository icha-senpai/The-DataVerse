...
use XFMG\Template\TemplaterSetup;
...
class Listener
{
	...
	public static function appSetup(App $app)
	{
		...
		$container['customFields.xfmgMediaFields'] = $app->fromRegistry(
			'xfmgMediaFields',
			function (Container $c) { return $c['em']->getRepository('XFMG:MediaField')->rebuildFieldCache(); },
	...
	public static function templaterTemplatePreRenderPublicEditor(Templater $templater, &$type, &$template, array &$params)
	{
	...
	public static function editorDialog(array &$data, AbstractController $controller)
	{
	...
	public static function importImporterClasses(Import $container, Container $parentContainer, array &$importers)
	{
		$importers = array_merge(
			$importers,
			Manager::getImporterShortNamesForType('XFMG')
		);
	}
