...
c=[];for(const d of a)c.push(b(d));await Promise.allSettled(c)},async preProcessFile(a){if("undefined"!==typeof ExifReader&&a.file.type.startsWith("image/")){try{var b=await ExifReader.load(a.file)}catch(c){b=null}if(b){const c={};for(const [d,e]of Object.entries(b)){if("MakerNote"===d)continue;b=e.id;if(!b)continue;let f=Array.isArray(e.value)?e.value.join("/"):e.value;switch(d){case "ComponentsConfiguration":case "ExifVersion":case "FileSource":case "FlashpixVersion":case "InteroperabilityVersion":case "SceneType":case "UserComment":f=
e.description;break;case "GPSAltitude":f=e.value.join("/");break;case "GPSLatitude":case "GPSLongitude":case "GPSTimeStamp":f=e.value.map(g=>g.join("/"))}c[d]={id:b,value:f}}this.exifMap[a.uniqueIdentifier]=c}}if(this.options.resizeImages&&a.file.type.startsWith("image/"))try{const c=await XF.ImageTools.resize(a.file,this.maxImageWidth,this.maxImageHeight,this.options.resizeImagesConvertType&&"optimize"===XF.config.imageOptimization?"image/webp":null);a.file=c;a.name=c.fileName||c.name;a.size=c.size;
a.relativePath=c.relativePath||c.webkitRelativePath||c.name;a.bootstrap()}catch(c){}if(0<XF.config.uploadMaxFilesize&&a.size>XF.config.uploadMaxFilesize)return this.uploadError(a,this.addErrorToJson({},XF.phrase("uploaded_file_is_too_large_for_server_to_process"))),!1},uploadProgress(a){const b=this.fileMap[a.uniqueIdentifier];b&&(this.setUploading(!0),this.resizeProgress(b,a.progress()))},resizeProgress(a,b){b=Math.floor(100*b);a=a.querySelector(this.options.templateProgress);let c=a.querySelector("i");
c||(a.innerHTML="&nbsp;",c=XF.createElement("i",{},a));c.textContent=`${b}%`;c.style.width=`${b}%`},uploadSuccess(a,b,c){b=this.getObjectFromMessage(b);this.setUploading(!1);delete this.exifMap[a.uniqueIdentifier];b.status&&"error"==b.status?this.uploadError(a,b,c):b.attachment?this.insertUploadedRow(b.attachment,this.fileMap[a.uniqueIdentifier]):(b=this.addErrorToJson(b),this.uploadError(a,b,c))},setUploading(a){a=a?!0:!1;a!==this.isUploading&&((this.isUploading=a)?XF.trigger(this.target,"attachment-manager:upload-start"):
XF.trigger(this.target,"attachment-manager:upload-end"))},getObjectFromMessage(a){if(a instanceof Object)return a;try{return JSON.parse(a)}catch(b){return this.addErrorToJson({})}},addErrorToJson(a,b){a.status="error";a.errors=[null===b?XF.phrase("oops_we_ran_into_some_problems"):b];return a},insertUploadedRow(a,b){a=this.applyUploadTemplate(a);this.editor||this.removeInsertButtons(a);b?b.replaceWith(a):(this.legacyMode?this.filesContainer.classList.add("is-active"):this.container.classList.add("is-active"),
this.filesContainer.appendChild(a));XF.activate(a);XF.layoutChange();XF.trigger(a,XF.customEvent("attachment:row-inserted",{newHtml:a,attachmentManager:this}));this.toggleInsertMultiRow()},uploadError(a,b,c){b=this.getObjectFromMessage(b);this.setUploading(!1);delete this.exifMap[a.uniqueIdentifier];const d=this.fileMap[a.uniqueIdentifier];if(d&&b.errors){c=b.errors[0];if(!c)for(const e in b.errors){c=b.errors[e];break}d.querySelector(this.options.templateProgress).remove();d.querySelector(this.options.templateError).textContent=
c;d.classList.add("is-uploadError");delete this.fileMap[a.uniqueIdentifier];XF.DataStore.remove(d,"file")}else XF.defaultAjaxSuccessError(b,200,c.xhr),this.removeFileRow(d)},actionButtonClick(a){a.preventDefault();var b=a.target.closest(this.options.actionButton);a=b.getAttribute("data-action");const c=b.getAttribute("data-type");b=b.closest(this.options.fileRow);switch(a){case "thumbnail":case "full":this.insertAttachment(b,a,c);break;case "delete":this.deleteAttachment(b,c);break;case "cancel":this.cancelUpload(b)}},
checkboxClick(){const a=this.filesContainer.querySelectorAll('input[type="checkbox"]:checked').length;n.querySelectorAll(this.options.selectActionButton).forEach(b=>b.disabled=a?!1:!0)},selectToggleClick(a){a.preventDefault();this.setSelectActionState(!this.container.classList.contains("is-selecting"));a.target.blur()},setSelectActionState(a){const b=this.container;b.classList.contains("is-selecting")!==a&&(b.querySelectorAll(this.options.selectToggleButton).forEach(c=>{const d=c.getAttribute("data-toggle"),
e=c.textContent;c.textContent=d;c.setAttribute("data-toggle",e)}),b.classList[a?"add":"remove"]("is-selecting"))},selectActionClick(a){a.preventDefault();const b=a.target.closest("button").getAttribute("data-action"),c=this.options.fileRow,d=this.options.actionButton,e=this.filesContainer.querySelectorAll(`${c} input[type="checkbox"]:checked`);e.forEach(f=>{f=f.closest(c).querySelectorAll(d);for(const g of f)if(f=g.dataset.type,("video"===f||"audio"===f)&&"thumbnail"===b){if("full"===g.dataset.action){g.click();
break}}else if(g.dataset.action===b){g.click();break}e.forEach(g=>g.checked=!1)});this.container.querySelector(this.options.insertMultiRow).querySelector('input[data-xf-init="check-all"]').checked=!1;this.setSelectActionState(!1)},insertAttachment(a,b,c){c=c||"image";const d=a.dataset.attachmentId;if(d&&this.editor){var e,f=null==(e=a.querySelector(this.options.templateThumb))?void 0:e.getAttribute("src");a=a.querySelector(this.options.templateView).getAttribute("href");var g;e={id:d,img:f};if("video"==
c||"audio"==c)b="full";if("full"==b)b=`[ATTACH=full]${d}[/ATTACH]`,"image"==c?g='<img src="{{img}}" data-attachment="full:{{id}}" alt="" />':"video"==c?g='<span contenteditable="false" draggable="true" class="fr-video fr-dvi fr-draggable fr-deletable"><video data-xf-init="video-init" data-attachment="full:{{id}}" src="{{img}}" controls></video></span>':"audio"==c&&(g='<span contenteditable="false" draggable="true" class="fr-audio fr-dvi fr-draggable fr-deletable"><audio data-attachment="full:{{id}}" src="{{img}}" controls></audio></span>&nbsp;'),
