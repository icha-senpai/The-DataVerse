        ...
        function flattenDiagnosticMessageText(diag, newLine, indent = 0) {
            ...
            if (diag.next) {
                for (const kid of diag.next) {
                    result += flattenDiagnosticMessageText(
                        kid,
                        newLine,
                        indent
                    );
        ...
        var DiagnosticsAdapter = class extends Adapter {
            ...
            _tsDiagnosticCategoryToMarkerSeverity(category) {
                switch (category) {
                    ...
                    case 0 /* Warning */:
                        return monaco_editor_core_exports.MarkerSeverity
                            .Warning;
                    case 2 /* Suggestion */:
        ...
        var SuggestAdapter = class _SuggestAdapter extends Adapter {
            ...
            async provideCompletionItems(model, position, _context, token) {
                ...
                const suggestions = info.entries.map((entry) => {
                    ...
                    const tags = [];
                    if (
                        entry.kindModifiers !== void 0 &&
                        entry.kindModifiers.indexOf("deprecated") !== -1
                    ) {
                        tags.push(
                            monaco_editor_core_exports.languages
            ...
            async resolveCompletionItem(item, token) {
                ...
                return {
                    ...
                    documentation: {
                        value: _SuggestAdapter.createDocumentationString(
                            details
                        ),
                    },
                };
...
${tagToString(tag)}`;
        ...
        var SignatureHelpAdapter = class _SignatureHelpAdapter extends Adapter {
            ...
            static _toSignatureHelpTriggerReason(context) {
                switch (context.triggerKind) {
                    case monaco_editor_core_exports.languages
            ...
            async provideSignatureHelp(model, position, token, context) {
                ...
                const info = await worker.getSignatureHelpItems(
                    ...
                    {
                        triggerReason:
                            _SignatureHelpAdapter._toSignatureHelpTriggerReason(
                                context
                            ),
                    }
                );
        ...
        var DefinitionAdapter = class extends Adapter {
            ...
            async provideDefinition(model, position, token) {
                ...
                for (let entry of entries) {
                    const refModel = this._libFiles.getOrCreateModel(
                        entry.fileName
                    );
                    if (refModel) {
                        result.push({
        ...
        var OutlineAdapter = class extends Adapter {
            async provideDocumentSymbols(model, token) {
                ...
                const root = await worker.getNavigationTree(
                    resource.toString()
                );
                ...
                const convert = (item, containerLabel) => {
                    const result2 = {
                        ...
                        kind:
                            ...
                            monaco_editor_core_exports.languages.SymbolKind
                                .Variable,
        ...
        var FormatAdapter = class extends FormatHelper {
            ...
            async provideDocumentRangeFormattingEdits(
                ...
                return edits.map((edit) =>
                    this._convertTextChanges(model, edit)
                );
            }
        };
        ...
        var CodeActionAdaptor = class extends FormatHelper {
            async provideCodeActions(model, range, context, token) {
                ...
                const actions = codeFixes
                    .filter((fix) => {
                        return (
                            fix.changes.filter((change) => change.isNewFile)
                                .length === 0
                        );
                    })
                    .map((fix) => {
            ...
            async provideRenameEdits(model, position, newName, token) {
                ...
                for (const renameLocation of renameLocations) {
                    ...
                    if (model2) {
                        ...
                        throw new Error(
                            `Unknown file ${renameLocation.fileName}.`
                        );
                    }
                }
                return { edits };
