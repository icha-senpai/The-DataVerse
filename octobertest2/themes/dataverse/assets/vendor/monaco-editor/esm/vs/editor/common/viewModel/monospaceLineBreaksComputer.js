...
export class MonospaceLineBreaksComputerFactory {
    static create(options) {
        return new MonospaceLineBreaksComputerFactory(
            options.get(132 /* EditorOption.wordWrapBreakBeforeCharacters */),
            options.get(131 /* EditorOption.wordWrapBreakAfterCharacters */)
        );
    }
    ...
    constructor(BREAK_BEFORE, BREAK_AFTER) {
        ...
        for (let i = 0; i < BREAK_BEFORE.length; i++) {
            this.set(
                ...
                1 /* CharacterClass.BREAK_BEFORE */
            );
        }
        for (let i = 0; i < BREAK_AFTER.length; i++) {
            this.set(
                BREAK_AFTER.charCodeAt(i),
                2 /* CharacterClass.BREAK_AFTER */
            );
        }
    }
    get(charCode) {
