...
let DropIntoEditorController =
    (DropIntoEditorController_1 = class DropIntoEditorController extends (
        Disposable
    ) {
        static get(editor) {
        ...
        async onDropIntoEditor(editor, position, dragEvent) {
            ...
            (_a = this._currentOperation) === null || _a === void 0
                ? void 0
                : _a.cancel();
            editor.focus();
            ...
            const p = createCancelablePromise(async (token) => {
                ...
                try {
                    const ourDataTransfer = await this.extractDataTransferData(
                        dragEvent
                    );
                    if (
            ...
            const results = await raceCancellation(
                Promise.all(
                    providers.map(async (provider) => {
                        try {
                            const edit =
                                await provider.provideDocumentOnDropEdits(
                                    model,
                                    position,
                                    dataTransfer,
                                    tokenSource.token
        ...
        async extractDataTransferData(dragEvent) {
            ...
            if (
                this.treeItemsTransfer.hasData(
