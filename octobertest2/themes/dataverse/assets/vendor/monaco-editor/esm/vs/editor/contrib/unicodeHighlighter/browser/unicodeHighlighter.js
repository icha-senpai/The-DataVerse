...
let UnicodeHighlighter = class UnicodeHighlighter extends Disposable {
    ...
    _updateHighlighter() {
        ...
        const highlightOptions = {
            ...
            allowedCodePoints: Object.keys(options.allowedCharacters).map((c) =>
                c.codePointAt(0)
            ),
...
export class ShowExcludeOptions extends EditorAction {
    ...
    async run(accessor, editor, args) {
        ...
        if (reason.kind === 0 /* UnicodeHighlighterReasonKind.Ambiguous */) {
            ...
            reason.kind === 1 /* UnicodeHighlighterReasonKind.Invisible */
        ) {
            ...
            reason.kind === 2 /* UnicodeHighlighterReasonKind.NonBasicAscii */
        ) {
            const action =
                new DisableHighlightingOfNonBasicAsciiCharactersAction();
            options.push({
...
async function excludeLocaleFromBeingHighlighted(
    ...
    var _a;
    const existingValue =
        (_a = configurationService.inspect(
