...
function _renderFormattedText(
    ...
    if (treeNode.type === 2 /* FormatType.Text */) {
        ...
        treeNode.type === 7 /* FormatType.Code */ &&
        renderCodeSegments
    ) {
...
function parseFormattedText(content, parseCodeSegments) {
    ...
    while (!stream.eos()) {
        ...
        if (
            ...
            if (
                current.type === type ||
                (current.type === 5 /* FormatType.Action */ &&
                    type === 6) /* FormatType.ActionClose */
            ) {
                current = stack.pop();
            } else {
                const newCurrent = {
...
function isFormatTag(char, supportCodeSegments) {
    return (
        formatTagType(char, supportCodeSegments) !== 0 /* FormatType.Invalid */
    );
}
