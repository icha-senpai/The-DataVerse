...
let BrowserClipboardService =
    (BrowserClipboardService_1 = class BrowserClipboardService extends (
        Disposable
    ) {
        ...
        async writeResources(resources) {
            if (resources.length === 0) {
                ...
                this.resourcesStateHash =
                    await this.computeResourcesStateHash();
            }
        }
        async readResources() {
        ...
        async computeResourcesStateHash() {
            ...
            return hash(
                clipboardText.substring(
                    0,
                    BrowserClipboardService_1.MAX_RESOURCE_STATE_SOURCE_LENGTH
                )
            );
        }
        clearResources() {
