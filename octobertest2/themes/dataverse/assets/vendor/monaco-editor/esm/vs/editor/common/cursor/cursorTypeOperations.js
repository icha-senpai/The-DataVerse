...
export class TypeOperations {
    ...
    static _distributePasteToCursors(
        config,
        selections,
        text,
        pasteOnNewLine,
        ...
        if (config.multiCursorPaste === "spread") {
            ...
            // Remove trailing \n if present
            if (
                text.charCodeAt(text.length - 1) === 10 /* CharCode.LineFeed */
            ) {
    ...
    static _replaceJumpToNextIndent(
        config,
        model,
        selection,
    ...
    static _runAutoClosingOvertype(
        prevEditOperationType,
        config,
        model,
        selections,
    ...
    static _getAutoClosingPairClose(
        config,
        model,
        selections,
        ch,
    ...
    static _runSurroundSelectionType(
        prevEditOperationType,
        config,
        model,
        selections,
    ...
    static _typeInterceptorElectricChar(
        prevEditOperationType,
        config,
        model,
        selection,
...
function getTypingOperation(typedText, previousTypingOperation) {
    if (typedText === " ") {
        return previousTypingOperation ===
...
function shouldPushStackElementBetween(
    ...
    if (
        previousTypingOperation === 5 /* EditOperationType.TypingFirstSpace */
    ) {
        // `abc |d`: No undo stop
        // `abc  |d`: Undo stop
        return false;
