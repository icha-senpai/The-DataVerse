...
class FastRenderedViewLine {
    constructor(domNode, renderLineInput, characterMapping) {
        ...
        const keyColumnCount = Math.floor(
            renderLineInput.lineContent.length /
                300 /* Constants.MaxMonospaceDistance */
        );
        if (keyColumnCount > 0) {
    ...
    _getColumnPixelOffset(lineNumber, column, context) {
        ...
        const keyColumnOrdinal =
            Math.floor(
                (column - 1) / 300 /* Constants.MaxMonospaceDistance */
            ) - 1;
        const keyColumn =
            (keyColumnOrdinal + 1) * 300 /* Constants.MaxMonospaceDistance */ +
            1;
