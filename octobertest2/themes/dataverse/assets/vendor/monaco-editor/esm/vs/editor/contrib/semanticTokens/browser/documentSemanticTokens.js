...
let ModelSemanticColoring =
    (ModelSemanticColoring_1 = class ModelSemanticColoring extends Disposable {
        constructor(
            model,
        ...
        _fetchDocumentSemanticTokensNow() {
            ...
            if (
                !hasDocumentSemanticTokensProvider(this._provider, this._model)
            ) {
            ...
            const contentChangeListener = this._model.onDidChangeContent(
                (e) => {
            ...
            request.then(
                (res) => {
                    ...
                    if (!res) {
                        ...
                        const styling =
                            this._semanticTokensStylingService.getStyling(
                                provider
                            );
                        this._setDocumentSemanticTokens(
                            provider,
            ...
            if (isSemanticTokensEdits(tokens)) {
                ...
                if (tokens.edits.length === 0) {
                    ...
                    for (const edit of tokens.edits) {
                        deltaLength +=
                            (edit.data ? edit.data.length : 0) -
                            edit.deleteCount;
                    }
                    const srcData = currentResponse.data;
                    const destData = new Uint32Array(
                        srcData.length + deltaLength
                    );
                    ...
                    for (let i = tokens.edits.length - 1; i >= 0; i--) {
                        ...
                        if (edit.start > srcData.length) {
                            ...
                            this._model.tokenization.setSemanticTokens(
                                null,
                                true
                            );
                            return;
