...
export class CharacterMapping {
    static getPartIndex(partData) {
        return (
            (partData &
                4294901760) /* CharacterMappingConstants.PART_INDEX_MASK */ >>>
    ...
    static getCharIndex(partData) {
        return (
            (partData &
                65535) /* CharacterMappingConstants.CHAR_INDEX_MASK */ >>>
...
function resolveRenderLineInput(input) {
    ...
    if (
        ...
        (input.renderWhitespace === 2 /* RenderWhitespace.Selection */ &&
...
function splitLargeTokens(lineContent, tokens, onlyAtSpaces) {
    ...
    if (onlyAtSpaces) {
        ...
        for (let i = 0, len = tokens.length; i < len; i++) {
            ...
            if (
                lastTokenEndIndex + 50 /* Constants.LongToken */ <
                tokenEndIndex
            ) {
                ...
                for (let j = lastTokenEndIndex; j < tokenEndIndex; j++) {
                    ...
                    if (
                        lastSpaceOffset !== -1 &&
                        j - currTokenStart >= 50 /* Constants.LongToken */
                    ) {
        ...
        for (let i = 0, len = tokens.length; i < len; i++) {
            ...
            if (diff > 50 /* Constants.LongToken */) {
                ...
                const piecesCount = Math.ceil(
                    diff / 50 /* Constants.LongToken */
                );
                for (let j = 1; j < piecesCount; j++) {
                    const pieceEndIndex =
                        lastTokenEndIndex + j * 50 /* Constants.LongToken */;
                    result[resultLen++] = new LinePart(
                        pieceEndIndex,
                        tokenType,
                        tokenMetadata,
                ...
                if (wasInWhitespace) {
                    ...
                    const nextChCode =
                        charIndex + 1 < len
                            ...
                            : 0 /* CharCode.Null */;
                    isInWhitespace =
                        nextChCode === 32 /* CharCode.Space */ ||
                        nextChCode === 9 /* CharCode.Tab */;
                }
            } else {
                isInWhitespace = true;
            }
...
function _renderLine(input, sb) {
    ...
    for (
        ...
        const partRendersWhitespace =
            renderWhitespace !== 0 /* RenderWhitespace.None */ &&
            part.isWhitespace();
