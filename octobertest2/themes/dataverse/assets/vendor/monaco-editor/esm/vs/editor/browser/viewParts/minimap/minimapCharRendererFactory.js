...
export class MinimapCharRendererFactory {
    ...
    static createSampleData(fontFamily) {
        ...
        for (const code of allCharCodes) {
            ctx.fillText(
                ...
                x,
                16 /* Constants.SAMPLED_CHAR_HEIGHT */ / 2
            );
    ...
    static _downsampleChar(source, sourceOffset, dest, destOffset, scale) {
        ...
        for (let y = 0; y < height; y++) {
            ...
            const sourceY1 =
                (y / height) * 16; /* Constants.SAMPLED_CHAR_HEIGHT */
            const sourceY2 =
                ((y + 1) / height) * 16; /* Constants.SAMPLED_CHAR_HEIGHT */
            for (let x = 0; x < width; x++) {
                const sourceX1 =
                    (x / width) * 10; /* Constants.SAMPLED_CHAR_WIDTH */
                const sourceX2 =
                    ((x + 1) / width) * 10; /* Constants.SAMPLED_CHAR_WIDTH */
                ...
                for (let sy = sourceY1; sy < sourceY2; sy++) {
                    const sourceRow =
                        sourceOffset +
                        Math.floor(sy) *
                            3840; /* Constants.RGBA_SAMPLED_ROW_WIDTH */
    ...
    static _downsample(data, scale) {
        ...
        for (
            let charIndex = 0;
            charIndex < 96 /* Constants.CHAR_COUNT */;
            charIndex++
        ) {
            brightest = Math.max(
                brightest,
