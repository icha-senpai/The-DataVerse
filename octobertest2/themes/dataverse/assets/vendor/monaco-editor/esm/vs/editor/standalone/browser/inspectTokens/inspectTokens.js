...
let InspectTokensController =
    (InspectTokensController_1 = class InspectTokensController extends (
        Disposable
    ) {
        static get(editor) {
        ...
        constructor(editor, standaloneColorService, languageService) {
            ...
            this._register(
                TokenizationRegistry.onDidChange((e) => this.stop())
            );
            this._register(
                this._editor.onKeyUp(
                    (e) => e.keyCode === 9 /* KeyCode.Escape */ && this.stop()
                )
            );
        }
        dispose() {
