    ...
    const disposables = [
        ...
        monaco.languages.registerDefinitionProvider("yaml", {
            async provideDefinition(model, position) {
                const worker = await workerManager.getWorker(model.uri);
                const locationLinks = await worker.doDefinition(
                    String(model.uri),
                    fromPosition(position)
                );
                return locationLinks == null
                    ? void 0
                    : locationLinks.map(toLocationLink);
            },
        }),
        monaco.languages.registerDocumentSymbolProvider("yaml", {
            ...
            async provideDocumentSymbols(model) {
                const worker = await workerManager.getWorker(model.uri);
                const items = await worker.findDocumentSymbols(
                    String(model.uri)
                );
        ...
        monaco.languages.registerCodeActionProvider("yaml", {
            async provideCodeActions(model, range, context) {
                ...
                const codeActions = await worker.getCodeAction(
                    ...
                    {
                        diagnostics:
                            ((_a = diagnosticMap.get(model)) == null
                                ? void 0
                                : _a.filter((diagnostic) =>
        ...
        monaco.languages.registerFoldingRangeProvider("yaml", {
            async provideFoldingRanges(model) {
                const worker = await workerManager.getWorker(model.uri);
                const foldingRanges = await worker.getFoldingRanges(
                    String(model.uri)
                );
                return foldingRanges == null
                    ? void 0
                    : foldingRanges.map(toFoldingRange);
            },
        }),
        ...
        monaco.languages.registerSelectionRangeProvider("yaml", {
            async provideSelectionRanges(model, positions) {
                ...
                return selectionRanges == null
                    ? void 0
                    : selectionRanges.map(toSelectionRanges);
            },
        }),
    ];
    return {
        ...
        async update(newOptions) {
            workerManager.updateCreateData(
                Object.assign(createData, newOptions)
            );
