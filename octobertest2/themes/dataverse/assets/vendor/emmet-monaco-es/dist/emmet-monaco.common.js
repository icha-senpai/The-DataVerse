...
function element$2(scanner, options) {
  ...
  while (readable$1(scanner)) {
    ...
    if (!elem.repeat && !isEmpty(elem) && consume$2(scanner, isRepeater)) {
      ...
      (attr =
        shortAttribute(scanner, 'id', options) || shortAttribute(scanner, 'class', options) || attributeSet(scanner))
    ) {
      if (!elem.attributes) {
...
function insertHref(node, text) {
  ...
  const hrefAttribute =
    (_a = node.attributes) === null || _a === void 0 ? void 0 : _a.find((attr) => attr.name === 'href')
  if (!hrefAttribute) {
    if (!node.attributes) {
      node.attributes = []
    }
    ...
    if (node.selfClosing && !node.children.length && !node.value) {
      ...
      if (!pushSnippet(node, state, next)) {
        ...
        if (!node.value && !node.children.length) {
          const innerFormat =
            config.options['output.formatLeafNode'] || config.options['output.formatForce'].includes(node.name)
          innerFormat && pushNewline(state.out, ++out.level)
...
function startsWithBlockTag(value, config) {
  if (value.length && typeof value[0] === 'string') {
    ...
    if (
      (matches === null || matches === void 0 ? void 0 : matches.length) &&
...
function parse(abbr, config) {
  var _a
  const snippets =
    ((_a = config.cache) === null || _a === void 0 ? void 0 : _a.stylesheetSnippets) || convertSnippets(config.snippets)
  const result = []
  if (config.cache) {
...
function mergedData(type, syntax, key, config, globals = {}) {
  ...
  return Object.assign(
    Object.assign(
      Object.assign(
        Object.assign(
          Object.assign(Object.assign({}, defaultConfig[key]), typeDefaults && typeDefaults[key]),
  ...
  if (isStyleSheetRes) {
    ...
    if (expandedAbbr && expandedText.length) {
      ...
      completionItems = makeSnippetSuggestion(
        monaco,
        stylesheetCustomSnippetsKeys !== null && stylesheetCustomSnippetsKeys !== void 0
      ...
      if (
        !completionItems.find(
          (x) => x.insertText === (expandedAbbr === null || expandedAbbr === void 0 ? void 0 : expandedAbbr.insertText),
        )
      ) {
    ...
    if (
      ...
      (expandedAbbr === null || expandedAbbr === void 0 ? void 0 : expandedAbbr.insertText) ===
...
function getExpandOptions(syntax, filter) {
  ...
  const snippets =
    type === 'stylesheet'
      ? (_a = customSnippetsRegistry[syntax]) !== null && _a !== void 0
        ? _a
...
function getTokenizationEnv(model) {
  ...
  // monaco-editor <= 0.34.0
  let _tokenizationStateStore =
    _tokenization === null || _tokenization === void 0 ? void 0 : _tokenization._tokenizationStateStore
  // monaco-editor >= 0.35.0
...
function isValidLocationForEmmetAbbreviation(model, position, syntax, language) {
  ...
  const state =
    ((_a = _stateStore.getBeginState) === null || _a === void 0
      ? void 0
      : _a.call(_stateStore, lineNumber - 1).clone()) || _stateStore.getStartState(lineNumber).clone()
